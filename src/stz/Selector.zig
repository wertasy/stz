...
pub fn start(self: *Selector, term: *Terminal, col: usize, row: usize, snap_mode: SelectionSnap) void {
    ...
    // 修正点击位置：如果点击的是宽字符的右半部分（wide_dummy），则修正为左半部分
    // 这样可以确保选择范围包含整个宽字符
    var corrected_col = col;
    const line = term.getVisibleLine(row);
    if (col < line.len and line[col].attr.wide_dummy) {
        if (corrected_col > 0) corrected_col -= 1;
    }

    sel.oe.x = corrected_col;
    sel.oe.y = row;
    sel.ob.x = corrected_col;
    sel.ob.y = row;
...
pub fn extend(self: *Selector, term: *Terminal, col: usize, row: usize, sel_type: SelectionType, done: bool) void {
    ...
    // 修正拖动位置：如果拖动到宽字符的右半部分（wide_dummy），则视为拖动到了该字符
    // 对于结束点，如果是从左向右选择，宽字符右侧（dummy）被选中意味着整个宽字符都被选中
    // 我们的 normalize 逻辑是包含性的 [nb, ne]，所以只要 ne 指向宽字符的任意一部分（包括 dummy），
    // 实际上通常 ne 会指向 dummy (如果鼠标在那)，但我们的 normalize 计算 ne.x
    // 如果 ne.x 是 dummy，isSelected 检查时：
    // isSelected(x) 判断 x 是否在 [nb, ne]
    // 宽字符的主体在 x-1。如果 ne=x，则 x-1 也在范围内（假设 nb < x-1）。
    // 但如果反向选择（从右向左），nb=x (dummy)。那么 x-1 不在范围内。
    // 所以这里也应该修正为宽字符的起始位置。
    var corrected_col = col;
    const line = term.getVisibleLine(row);
    if (col < line.len and line[col].attr.wide_dummy) {
...
pub fn normalize(self: *Selector, term: *Terminal) void {
    ...
    // 如果是普通选择模式，且结束点是宽字符（的左半部分），扩展选择范围以包含其右半部分（占位符）
    // 这样整个宽字符都会被高亮显示
    if (sel.type == .regular and sel.ne.y < term.row) {
        const line = term.getVisibleLine(sel.ne.y);
        if (sel.ne.x < line.len and line[sel.ne.x].attr.wide) {
            if (sel.ne.x + 1 < term.col) {
                sel.ne.x += 1;
            }
        }
    }
