...
> ⚠️ **开发状态**: 核心功能（PTY、VT100解析、X11渲染）已工作，但处于早期开发阶段。
> 
> **已知限制**: 输入法 (IME) 尚未完全集成，OSC 52 剪贴板暂未连通。
...
### 已实现 ✅
 ...
 - ✅ **Box Drawing**: 内置制表符完美绘制（不依赖字体）

### 待实现 ⚠️

详见 [TODO.md](TODO.md)。

## ⚖️ 与 st 的差异 (Differences from st)

虽然 `stz` 旨在复刻 `st` 的极简主义，但在实现上做了一些现代化的权衡：

- **Box Drawing**: `stz` 内置了手动绘制制表符的逻辑，不依赖字体（类似 `st` 的 `boxdraw` 补丁），确保 TUI 界面线条完美连接。
- **模块化**: 代码被拆分为多个职责单一的 Zig 模块，而非单文件结构。
