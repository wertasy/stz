...
fn drawBoxChar(self: *Renderer, u: u21, x: i32, y: i32, w: i32, h: i32, color: *x11.XftColor, bg_color: *x11.XftColor, bold: bool) !void {
    const data = boxdraw.getDrawData(u);
    if (data == 0) return;
    const mwh = @min(w, h);
    ...
    const cat = data & ~@as(u16, boxdraw.BOLD | 0xff);
    if (cat == boxdraw.TRIANGLE) {
    ...
    if (cat == boxdraw.BLOCK_DOWN) {
        const d = @divTrunc(@as(i32, @intCast(data & 0xFF)) * h + 4, 8);
        x11.XftDrawRect(self.draw, color, x, y + d, @intCast(w), @intCast(h - d));
        return;
    } else if (cat == boxdraw.BLOCK_UPPER) {
        const d = @divTrunc(@as(i32, @intCast(data & 0xFF)) * h + 4, 8);
        x11.XftDrawRect(self.draw, color, x, y, @intCast(w), @intCast(d));
        return;
    } else if (cat == boxdraw.BLOCK_LEFT) {
        const d = @divTrunc(@as(i32, @intCast(data & 0xFF)) * w + 4, 8);
        x11.XftDrawRect(self.draw, color, x, y, @intCast(d), @intCast(h));
        return;
    } else if (cat == boxdraw.BLOCK_RIGHT) {
        const d = @divTrunc(@as(i32, @intCast(data & 0xFF)) * w + 4, 8);
        x11.XftDrawRect(self.draw, color, x + d, y, @intCast(w - d), @intCast(h));
        return;
    }
    if (cat == boxdraw.BLOCK_QUADRANT) {
        const qw = @divTrunc(w + 1, 2);
        const qh = @divTrunc(h + 1, 2);
        if (data & boxdraw.TOP_LEFT != 0) x11.XftDrawRect(self.draw, color, x, y, @intCast(qw), @intCast(qh));
        if (data & boxdraw.TOP_RIGHT != 0) x11.XftDrawRect(self.draw, color, x + qw, y, @intCast(w - qw), @intCast(qh));
        if (data & boxdraw.BOTTOM_LEFT != 0) x11.XftDrawRect(self.draw, color, x, y + qh, @intCast(qw), @intCast(h - qh));
        if (data & boxdraw.BOTTOM_RIGHT != 0) x11.XftDrawRect(self.draw, color, x + qw, y + qh, @intCast(w - qw), @intCast(h - qh));
        return;
    }
    if (data & boxdraw.BLOCK_SHADE != 0) {
    ...
    if (data & (boxdraw.LINE | boxdraw.ARC) != 0) {
        const light = data & (boxdraw.LIGHT_LEFT | boxdraw.LIGHT_UP | boxdraw.LIGHT_RIGHT | boxdraw.LIGHT_DOWN);
        const double_ = data & (boxdraw.DOUBLE_LEFT | boxdraw.DOUBLE_UP | boxdraw.DOUBLE_RIGHT | boxdraw.DOUBLE_DOWN);
        if (light != 0) {
            const arc = data & boxdraw.ARC != 0;
            ...
            // // 先绘制直线（如果有）
            if (data & boxdraw.LIGHT_LEFT != 0) x11.XftDrawRect(self.draw, color, x, midy, @intCast(w2_line + s + d_len), @intCast(s));
            if (data & boxdraw.LIGHT_UP != 0) x11.XftDrawRect(self.draw, color, midx, y, @intCast(s), @intCast(h2_line + s + d_len));
            if (data & boxdraw.LIGHT_RIGHT != 0) x11.XftDrawRect(self.draw, color, midx - d_len, midy, @intCast(w - w2_line + d_len), @intCast(s));
            if (data & boxdraw.LIGHT_DOWN != 0) x11.XftDrawRect(self.draw, color, midx, midy - d_len, @intCast(s), @intCast(h - h2_line + d_len));

            // // 绘制圆角弧
            // if (arc and corner_radius > 0) {
            //     // ╭ (U+256D, BDA+LD+LR): 左上圆角，边线向下和向右
            //     if ((data & boxdraw.LD) != 0 and (data & boxdraw.LR) != 0) {
            //         try self.drawBoxArc(x, y, w, h, s, color, corner_radius, 0);
            //     }
            //     // ╮ (U+256E, BDA+LD+LL): 右上圆角，边线向下和向左
            //     if ((data & boxdraw.LD) != 0 and (data & boxdraw.LL) != 0) {
            //         try self.drawBoxArc(x, y, w, h, s, color, corner_radius, 1);
            //     }
            //     // ╯ (U+256F, BDA+LU+LL): 右下圆角，边线向上和向左
            //     if ((data & boxdraw.LU) != 0 and (data & boxdraw.LL) != 0) {
            //         try self.drawBoxArc(x, y, w, h, s, color, corner_radius, 2);
            //     }
            //     // ╰ (U+2570, BDA+LU+LR): 左下圆角，边线向上和向右
            //     if ((data & boxdraw.LU) != 0 and (data & boxdraw.LR) != 0) {
            //         try self.drawBoxArc(x, y, w, h, s, color, corner_radius, 3);
            //     }
            // }
        }
        if (double_ != 0) {
            const dl = data & boxdraw.DOUBLE_LEFT != 0;
            const du = data & boxdraw.DOUBLE_UP != 0;
            const dr = data & boxdraw.DOUBLE_RIGHT != 0;
            const dd = data & boxdraw.DOUBLE_DOWN != 0;
            if (dl) {
