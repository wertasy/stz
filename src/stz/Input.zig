...
pub fn sendMouseReport(self: *Input, x: usize, y: usize, button: u32, state: u32, event_type: u8) !void {
    ...
    // 修正鼠标点击位置：如果点击的是宽字符的右半部分（wide_dummy），则修正为左半部分
    // 这样应用程序接收到的坐标是宽字符的起始位置，避免光标定位错误（光标只显示一半）
    var report_x = x;
    const line = self.term.getVisibleLine(y);
    if (x < line.len and line[x].attr.wide_dummy) {
        if (report_x > 0) report_x -= 1;
    }
    ...
    if (self.term.mode.mouse_sgr) {
        ...
        const s = try std.fmt.bufPrint(&buf, "\x1b[<{d};{d};{d}{c}", .{ code, report_x + 1, y + 1, ch });
        _ = try self.pty.write(s);
    } else if (report_x < 223 and y < 223) {
        var buf: [6]u8 = undefined;
        buf[0] = 0x1b;
        buf[1] = '[';
        buf[2] = 'M';
        buf[3] = @as(u8, @intCast(32 + code));
        buf[4] = @as(u8, @intCast(32 + report_x + 1));
